...
- `ci/Dockerfile` — **通用基础镜像**（不挂任何特定 RTOS 名下）：qemu-system-arm
  + gdb-multiarch + arm-none-eabi-gcc + scons + uv。所有 RTOS × 硬件平台组合
...
- [ ] 把仓库镜像到 CNB（`git remote add cnb <您的cnb仓库地址>` 后 push）
- [ ] 首次 push 触发 lint + qemu-test 两条流水线
- [ ] 若两者均成功，本地端到端闭环完成
- [ ] 在 README 顶部嵌入 CNB badge
